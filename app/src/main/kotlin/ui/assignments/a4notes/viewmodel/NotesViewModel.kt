...
class NotesViewModel : ViewModel() {
    ...
    private val editNote: MutableLiveData<VMNote?> = MutableLiveData(null)

    private val searchStr = MutableLiveData("")
    ...
    init {
        ...
        val noteChangeCallback = object : OnPropertyChangedCallback() {
            override fun onPropertyChanged(modelNote: Observable?, propertyId: Int) {
                model.notes.find { it.id == propertyId }?.apply {// find MVNote in notes
                    modelNote as Model.ModelNote
                    notes.value?.find { it.value?.id == propertyId }?.apply {
                        this.value = VMNote(modelNote.id, modelNote.title, modelNote.content, modelNote.important, modelNote.archived)
                        if ((modelNote.archived && viewArchived.value == false) ||
                            (!modelNote.title.lowercase().contains(searchStr.value!!.lowercase()) &&
                             !modelNote.content.lowercase().contains(searchStr.value!!.lowercase()))) { // if note is archived and archived notes are not showing, remove from notes
                    ...
                    if (!modelNote.archived && viewArchived.value == false) {
                        notes.value = notes.value.apply {
                            this?.add(MutableLiveData(VMNote(modelNote)))
    ...
    fun setViewArchived(showArchived: Boolean) {
        ...
        model.notes.forEach {
            if (showArchived || !it.archived) {
                if (it.title.lowercase().contains(searchStr.value!!.lowercase()) ||
                    it.title.lowercase().contains(searchStr.value!!.lowercase())) {
                    notes.value = notes.value.apply {
                        this?.add(MutableLiveData(VMNote(it)))
                    }
                }
            }
        }
    ...
    fun getEditNote(): LiveData<VMNote> {
        return editNote as LiveData<VMNote>
    }
    ...
    fun filterVisible(searchStr: String) {
        notes.value?.clear()
        model.notes.forEach {
            if (viewArchived.value == true || !it.archived) {
                if (it.title.lowercase().contains(searchStr.lowercase()) ||
                    it.title.lowercase().contains(searchStr.lowercase())) {
                    notes.value = notes.value.apply {
                        this?.add(MutableLiveData(VMNote(it)))
                    }
                }
